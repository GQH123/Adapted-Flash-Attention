...
class SparsityConfig:
    ...
    def __init__(self, num_heads, block=16, different_layout_per_head=False, squeeze_head_dim=True):
...
class DenseSparsityConfig(SparsityConfig):
    ...
    def __init__(self, num_heads, block=16, different_layout_per_head=False, attention='unidirectional'):
...
def directional_scaling(blockmask, scale, direction='vertical'):
    if direction not in ['vertical', 'horizontal']:
        raise ValueError(f'Invalid direction {direction}, options are \'vertical\' or \'horizontal\'')
    return blockmask.repeat_interleave(scale, dim=-2 if direction == 'vertical' else -1)
