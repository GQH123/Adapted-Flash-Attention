...
class SparsityConfig:
    ...
    def __init__(self, num_heads, hdim, block, different_layout_per_head=False, squeeze_head_dim=True):
    ...
    def setup_layout(self, seq_len):
        ...
        if (seq_len % self.b_r != 0):
            raise ValueError(f'Sequence length, {seq_len}, needs to be dividable by Block Row Size {self.b_r}!')
        if (seq_len % self.b_c != 0):
            raise ValueError(f'Sequence length, {seq_len}, needs to be dividable by Block Column Size {self.b_c}!')
        num_blocks_row = seq_len // self.b_r
        num_blocks_col = seq_len // self.b_c
        # TODO Currently we allocate layout per head; needs to be updated if heads share a single layout.
        layout = torch.zeros((self.num_heads, num_blocks_row, num_blocks_col), dtype=torch.int64)
        return layout
...
class DenseSparsityConfig(SparsityConfig):
    ...
    def __init__(self, num_heads, hdim, block=16, different_layout_per_head=False):
        ...
        super().__init__(num_heads, hdim, block, different_layout_per_head)
    ...
    def make_layout(self, seq_len):
        ...
        layout[:, :, :] = 1
        return layout if not self.squeeze_head_dim or len(layout.shape) == 2 else layout[0]
...
class FixedSparsityConfig(SparsityConfig):
    ...
    def set_local_layout(self, h, layout):
        ...
        num_blocks_row = layout.shape[1]  # number of blocks in a COL
        num_blocks_col = layout.shape[2]  # number of blocks in a ROW
        for i in range(0, num_blocks_row, self.num_local_blocks):
            end = min(i + self.num_local_blocks, num_blocks_row)
            for row in range(i, end):
                for col in range(i, min(num_blocks_col, (row + 1 if self.attention == 'unidirectional' else end))):
                    layout[h, row, col] = 1
        return layout
    ...
    def set_global_layout(self, h, layout):
        ...
        end = num_blocks_col - (num_blocks_col % self.num_local_blocks)
        ...
        end = num_blocks_row - (num_blocks_row % self.num_local_blocks)
        if (self.horizontal_global_attention):
            for i in range(first_global_block_idx, end, self.num_local_blocks):
                layout[h, i:i + self.num_global_blocks, :] = 1

        # set last global blocks; handle possible short last local window
        if (end < num_blocks_col):
            start = min(end + first_global_block_idx, num_blocks_col - self.num_global_blocks)
        ...
        if (end < num_blocks_row):
            start = min(end + first_global_block_idx, num_blocks_row - self.num_global_blocks)
            end = start + self.num_global_blocks
...
class VariableSparsityConfig(SparsityConfig):
    ...
    def set_random_layout(self, h, layout):
        ...
        num_blocks_row = layout.shape[1]  # number of blocks in a COL
        num_blocks_col = layout.shape[2]  # number of blocks in a ROW
        if (num_blocks_row < self.num_random_blocks):
            raise ValueError(
                f'Number of random blocks, {self.num_random_blocks}, must be smaller than overall number of blocks in a col, {num_blocks_row}!'
            )
        if (num_blocks_col < self.num_random_blocks):
            raise ValueError(
                f'Number of random blocks, {self.num_random_blocks}, must be smaller than overall number of blocks in a row, {num_blocks_col}!'
            )
            
        for row in range(0, num_blocks_row):
            rnd_cols = random.sample(range(0, num_blocks_col), self.num_random_blocks)
    ...
    def set_local_layout(self, h, layout):
        ...
        for block_size in self.local_window_blocks:
            ...
            end_block_idx = min(end_block_idx, num_blocks_row)
            for row in range(start_block_idx, end_block_idx):
                for col in range(start_block_idx, min(num_blocks_col, (row + 1 if self.attention == 'unidirectional' else end_block_idx))):
                    layout[h, row, col] = 1
        ...
        for i in range(start_block_idx, num_blocks_row, block_size):
            end_block_idx = min(i + block_size, num_blocks_row)
            for row in range(i, end_block_idx):
                for col in range(i, min(num_blocks_col, (row + 1 if self.attention == 'unidirectional' else end_block_idx))):
                    layout[h, row, col] = 1
        return layout
    ...
    def set_global_layout(self, h, layout):
        ...
        if (self.global_block_end_indices is None):
            ...
            for _, (start_idx, end_idx) in enumerate(zip(self.global_block_indices, self.global_block_end_indices)):
                # if global block idx is in the range of the sequence blocks
                if (start_idx < num_blocks_row):
                    end_idx = min(end_idx, num_blocks_row)
                    #global rows
                    if (self.horizontal_global_attention):
            ...
            for _, (start_idx, end_idx) in enumerate(zip(self.global_block_indices, self.global_block_end_indices)):
                # if global block idx is in the range of the sequence blocks
                if (start_idx < num_blocks_col):
                    end_idx = min(end_idx, num_blocks_col)
                    #global columns
...
class BigBirdSparsityConfig(SparsityConfig):
    ...
    def set_random_layout(self, h, layout):
        ...
        for row in range(0, num_blocks_row):
            sample_range = range(0, num_blocks_col) if self.attention == 'bidirectional' else range(0, row + 1)
    ...
    def set_sliding_window_layout(self, h, layout):
        ...
        num_blocks_row = layout.shape[1]  # number of blocks in a COL
        num_blocks_col = layout.shape[2]  # number of blocks in a ROW
        if (num_blocks_row < self.num_sliding_window_blocks):
            raise ValueError(
                f'Number of sliding window blocks, {self.num_sliding_window_blocks}, must be smaller than overall number of blocks in a col, {num_blocks_row}!'
            )
        if (num_blocks_col < self.num_sliding_window_blocks):
            raise ValueError(
                f'Number of sliding window blocks, {self.num_sliding_window_blocks}, must be smaller than overall number of blocks in a row, {num_blocks_col}!'
            )

        w = self.num_sliding_window_blocks // 2
        for row in range(0, num_blocks_row):
            start = max(0, row - w)
            end = min(row + w + 1, num_blocks_col)
    ...
    def set_global_layout_itc(self, h, layout):
        ...
        num_blocks_row = layout.shape[1]  # number of blocks in a COL
        num_blocks_col = layout.shape[2]  # number of blocks in a ROW
        if (num_blocks_row < self.num_global_blocks):
            raise ValueError(
                f'Number of global blocks, {self.num_global_blocks}, must be smaller than overall number of blocks in a col, {num_blocks_row}!'
            )
        if (num_blocks_col < self.num_global_blocks):
            raise ValueError(
                f'Number of global blocks, {self.num_global_blocks}, must be smaller than overall number of blocks in a row, {num_blocks_col}!'
            )
...
class LocalSlidingWindowSparsityConfig(SparsityConfig):
    ...
    def __init__(self, num_heads, hdim, block=16, num_sliding_window_blocks=3, attention='bidirectional'):
        ...
        super().__init__(num_heads, hdim, block)
        self.num_sliding_window_blocks = num_sliding_window_blocks
        self.attention = attention
    ...
    def set_sliding_window_layout(self, h, layout):
        ...
        for row in range(0, num_blocks_row):
            start = max(0, row - w)
            end = min(row + w + 1, num_blocks_col) if self.attention == "bidirectional" else row + 1
            layout[h, row, start:end] = 1
        return layout
...
def get_sparsity_config_cls(sparsity_config):
    if sparsity_config not in ['dense', 'fixed', 'variable', 'bigbird', 'longformer', 'localslide']:
        raise ValueError(f'Unsupported sparsity config: {sparsity_config}')
    return {
        ...
        'localslide': LocalSlidingWindowSparsityConfig
    }[sparsity_config]